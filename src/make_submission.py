        ...
        for name, model in models.items():
            if name in ['trajectory', 'ensemble']:
                ...
                if name == 'ensemble':
                    ...
                    # Reshape to (batch_size * sequence_length, 32, 32, 64) as expected by VanillaLocalizationModel
                    flattened_data = tf.reshape(flattened_data, [-1, 32, 32, 64])
                    print(f"\n{name.capitalize()} model input shape (reshaped): {flattened_data.shape}")
